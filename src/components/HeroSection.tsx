...
const HeroSection = ({ movie, linkPrefix = "/movie" }: HeroSectionProps) => {
  ...
  return (
    <section className="relative w-full h-[60vh] sm:h-[70vh] md:h-[85vh] overflow-hidden">
      ...
      <div className="relative h-full container mx-auto px-4 flex flex-col justify-end pb-12 sm:pb-16 md:pb-24 max-w-4xl">
        <div className="space-y-3 sm:space-y-5">
          <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-7xl font-bold text-foreground tracking-tight animate-fade-in-up leading-tight">
            {movie.title}
          </h1>
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs sm:text-sm animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
            <div className="flex items-center gap-1 sm:gap-1.5 bg-primary/20 text-primary px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full">
          ...
          <div className="flex items-center gap-2 sm:gap-3 pt-1 animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
            <Link to={detailUrl}>
              <Button size="sm" className="gap-1.5 sm:gap-2 font-semibold shadow-lg shadow-primary/25 hover:shadow-primary/40 transition-shadow text-xs sm:text-sm h-9 sm:h-11 px-3 sm:px-6">
                <Play className="h-3.5 w-3.5 sm:h-4 sm:w-4 fill-current" /> Watch Trailer
              </Button>
            </Link>
            <Link to={detailUrl}>
              <Button size="sm" variant="outline" className="gap-1.5 sm:gap-2 bg-foreground/10 border-foreground/20 hover:bg-foreground/20 text-xs sm:text-sm h-9 sm:h-11 px-3 sm:px-6">
