...
const MovieDetails = () => {
  ...
  const movieJsonLd = {
    ...
    ...(trailer ? { trailer: { "@type": "VideoObject", name: `${movie.title} Trailer`, description: `Official trailer for ${movie.title}. ${movie.overview?.slice(0, 150) || ""}`, embedUrl: `https://www.youtube.com/embed/${trailer.key}`, thumbnailUrl: `https://img.youtube.com/vi/${trailer.key}/hqdefault.jpg`, uploadDate: movie.release_date || new Date().toISOString().split("T")[0], contentUrl: `https://www.youtube.com/watch?v=${trailer.key}` } } : {}),
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead
        title={`${movie.title}${year ? ` (${year})` : ""} – Download Subtitles, Watch Trailer & Cast | CinemaHub`}
        description={`Download ${movie.title} subtitles in 50+ languages (SRT). Watch trailer, browse cast, read reviews, and explore ratings on CinemaHub.`}
