    ...
    <section className="relative group/row">
      ...
      <div className="relative">
        <button onClick={() => scroll("left")}
          className="absolute left-0 top-0 bottom-12 z-10 w-8 sm:w-12 bg-gradient-to-r from-background via-background/80 to-transparent flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition-opacity duration-300 hidden sm:flex">
        ...
        <div ref={scrollRef} className="flex gap-2 sm:gap-3 overflow-x-auto scrollbar-hide px-4 md:px-0 pb-2 snap-x snap-mandatory sm:snap-none">
          {movies.map((movie) => (
        ...
        <button onClick={() => scroll("right")}
          className="absolute right-0 top-0 bottom-12 z-10 w-8 sm:w-12 bg-gradient-to-l from-background via-background/80 to-transparent flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition-opacity duration-300 hidden sm:flex">
          <div className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-card/80 border border-border/50 flex items-center justify-center hover:bg-muted transition-colors">
